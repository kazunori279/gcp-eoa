...
| **M2 · Scale** (Cloud Shell 2026-07-24, re-run 3) | ✅\* | ~17 min | **Step 3 code-exec hard gate worked first-try** (grep-confirmed `BuiltInCodeExecutor`); smoke green → **one clean deploy** of fresh engine `1836…` (~3 min); **4/4 deployed scenarios**. **\*Caveat (P3 open):** Memory Bank `…/memories` API still **0 rows** after settle — cross-session persistence not independently confirmed (likely session bleed) |
...
| **M5 · Engage** (Cloud Shell 2026-07-24, re-run 3) | ❌ | — | **NEW P1: the create-GE-app step failed.** With `transit-crisis` deleted first, agy ran create/register curls, **reported success + wrote `registration_results.md`, but no app/agent actually exists** (engines list empty, `engines.get` 401/404, no create op pending). Creating a GE app programmatically needs a data store + specific engine/assistant config — fix #5 must add an `engines.get==200` hard gate and a correct create sequence. Prior runs "passed" only because the app pre-existed. |
| **End-to-end** (Cloud Shell 2026-07-24, re-run 3) | **4 pass / 1 caveat / 1 fail** | ~2 hr | Fresh slate via `cleanup.sh --apps`. **M0–M4 pass** — fixes validated: M0 header (no patch), M2 code-exec hard gate (first-try), **M4 sim 10/10 no hang + eval 4/4 incl. grounding trap**, M3 SPIFFE+Model Armor. M2 caveat: Memory Bank API 0 rows (P3). **M5 FAILED**: create-GE-app step over-reported success; no app/agent actually created (new P1). One fresh deploy (engine `1836…`). |
