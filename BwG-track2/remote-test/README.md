...
| **M1 · Build** (Cloud Shell 2026-07-24, re-run 3) | ✅ | ~6 min | scaffold + 3 tools + 5-rule instruction (widened rule 3); playground on :8080; **4/4 headless scenarios** — S1→9038 reroute, S2 delay 9148, S3 on-time 9024, and **grounding trap** ("which platform" → "unavailable", no hallucination) |
| **M2 · Scale** (Cloud Shell 2026-07-24, re-run 3) | ✅\* | ~17 min | **Step 3 code-exec hard gate worked first-try**; smoke green → **one clean deploy** of fresh engine `1836…`; **4/4 deployed scenarios**. **\*P3 root-caused + fixed:** Memory Bank `…/memories` = **0 rows** because Step 2 added session state but **not the Memory Bank write/read** (`agent.py` had no `PreloadMemoryTool` / `generate_memories` callback). Added a Step-2 hard-gate self-check so it can't silently skip. |
| **M3 · Govern** (Cloud Shell 2026-07-24, re-run 3) | ✅ | ~1.5 min | **SPIFFE** identity for engine `1836…` + least-privilege PASS; fresh Model Armor `transit-shield` blocks DAN jailbreak (`MATCH_FOUND` HIGH) + credit-card PII, passes benign |
| **M4 · Optimize** (Cloud Shell 2026-07-24, re-run 3) | ✅ | ~10 min | **Both P1s fixed.** Step 2 crisis sim now **completes 10/10, no hang** (rewritten to drive via ADK Runner + per-scenario `asyncio.wait_for` guard). Step 3 `adk eval` **4/4 incl. grounding-trap PASS** (widened rule 3). |
| **M5 · Engage** (Cloud Shell 2026-07-24, re-run 3) | ✅ | ~2 min | **Create-GE-app step works.** With `transit-crisis` deleted first, agy **created** the GE app (data store → engine `APP_TYPE_INTRANET`) **and registered** the agent; verified via the **regional endpoint** — `engines.get` 200, "Transit-Crisis Agent" **ENABLED**. (An initial "failed" verdict was my harness bug: verifying against the plain `discoveryengine.googleapis.com` host returns empty/401; must use `global-discoveryengine.googleapis.com`. Fixed in `cleanup.sh` + `m5.html`.) |
| **End-to-end** (Cloud Shell 2026-07-24, re-run 3) | **5 pass / 1 caveat** | **~40 min** agy net (sum of module elapsed; facilitated workshop ~1h50m, or ~1h skipping M3/M4) | Fresh slate via `cleanup.sh --apps`. **All 5 agy modules pass** — fixes validated: M0 header (no patch), M2 code-exec hard gate (first-try), **M4 sim 10/10 no hang + eval 4/4 incl. grounding trap**, M3 SPIFFE+Model Armor, **M5 create-GE-app + register (ENABLED)**. M2 P3 (Memory Bank 0 rows) root-caused — Step 2 skipped the Memory Bank write/read; **fixed with a Step-2 hard gate**. One fresh deploy (engine `1836…`); agent published to GE. |
